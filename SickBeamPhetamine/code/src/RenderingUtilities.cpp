...
namespace render {
	...
	SDL_Window* createSDLWindow(const char* windowTitle, int width, int height) {
		...
		* (Need when two GPU with Optimus for example) */
		...
		SDLWindow = SDL_CreateWindow(windowTitle, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
		if (!SDLWindow) { /* Die if creation failed */
	...
	void VAO::loadToGPU(std::vector<GLfloat> &vertices, std::vector<GLfloat> &normals, std::vector<GLfloat> &colors, std::vector<GLushort> &faces) {
